...
For each backend, a DAG population is split into a ``fastpath`` cohort and a ``control`` cohort; each
task does cohort-dependent work and reports its own duration through ``track_outcome``. We then read
...
Backends exercised with their real libs:
  * flagd (real gRPC container)  -- assigns the cohort; has no analytics, so the outcome rides the
...
def do_work(dag_id: str) -> tuple[str, float]:
    """Cohort-dependent work; returns (cohort, measured_ms). fastpath takes the optimized branch."""
    fast = flag_enabled(FLAG, f"{dag_id}:work", dag_id=dag_id)
    cohort = "fastpath" if fast else "control"
    t = time.perf_counter()
    if fast:
    ...
    track_outcome("task_duration_ms", f"{dag_id}:work", value=round(ms, 3), variant=cohort, dag_id=dag_id)
    return cohort, ms


def _print_lift(title: str, samples: list[tuple[str, float]], readout_note: str) -> bool:
    by = defaultdict(list)
    for cohort, ms in samples:
        by[cohort].append(ms)
    print(f"\n[{title}]  readout: {readout_note}")
    print(f"    {'cohort':<10}{'runs':>6}{'mean ms':>11}{'median ms':>12}")
    means = {}
    for cohort in sorted(by):
        vals = by[cohort]
        means[cohort] = statistics.mean(vals)
        print(f"    {cohort:<10}{len(vals):>6}{means[cohort]:>11.1f}{statistics.median(vals):>12.1f}")
...
def main():
    print("=" * 84)
    print(f"MEASURE LOOP across real backends: {len(POPULATION)} DAGs, cohort-split work, outcome read")
